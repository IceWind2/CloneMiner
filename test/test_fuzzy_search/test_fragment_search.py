...
class TestFragmentSearch(unittest.TestCase):
    ...
    def test_strict_duplicates(self):
        ...
        self.searcher.config.fragment_size = 10
    ...
    def test_one_case_2(self):
        text_model = self.tokenizer.create_text_model("Two a bit different paragraphs. Algorithm should recognize this phrases as a clone. Two a little different paragraphs. Algorithm can recognize this phrases as a clone", self.config)
        self.searcher.config.fragment_size = 13
        duplicates = self.searcher.find_duplicates(text_model)
        self.assertEqual(1, len(duplicates.cases))
        self.assertEqual(2, len(duplicates.cases[0].text_fragments))
